    ...
    data.forEach(({ title, assignee, state, format, version_number, id, slug, language, important_note, time_in_queue, reviewer }, index) => {
      ...
      if (currentPage == "my-content") {
        ...
        if (state == "fact_check_received") {
          ...
          newRow.querySelector('.view-response').innerHTML = `<a href="content-item-history-and-notes?id=${id}&content-type=${format}&status=${status}" class="govuk-link govuk-link--no-visited-state">View response</a>`
